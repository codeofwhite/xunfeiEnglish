...
public class IseDemo extends WebSocketListener {
    ...
    private static final String appid = "30fa122e";//控制台获取
    private static final String apiSecret = "Mzg2ZDdjODJjYWM1MzE5M2ZkM2E0NGJl";//控制台获取
    private static final String apiKey = "96b41233a0974ee46f365d13ef6bea09";//控制台获取
    ...
    private static String text = "[choice]\n" +
            ...
            "cats\n" +
            "[question]\n" +
            "What did the woman dislike?";//评测试题,英文试题:[content]\nthere was a gentleman live near my house.
    private String file = "iseAudio/en/read_choice_en.pcm";//评测音频,如传mp3格式请改变参数aue的值为lame
            ...
            if (resp.getData() != null) {
                ...
                if (resp.getData().getStatus() == 2) {
                    try {
                        ...
                        result = parseXmlResult(decodedData);
                        System.out.println(result); // 确保这里打印出正确的结果
                        if (latch != null) {
                            latch.countDown(); // 释放锁
                        }
                    } catch (Exception e) {
    ...
    private String parseXmlResult(String xml) {
        try {
            ...
            NodeList readChapterList = root.getElementsByTagName("read_chapter");
            if (readChapterList.getLength() > 0) {
                Element readChapter = (Element) readChapterList.item(0);
                JsonObject resultJson = new JsonObject();
                resultJson.addProperty("AccuracyScore", readChapter.getAttribute("accuracy_score"));
                resultJson.addProperty("FluencyScore", readChapter.getAttribute("fluency_score"));
                resultJson.addProperty("IntegrityScore", readChapter.getAttribute("integrity_score"));
                resultJson.addProperty("PhoneScore", readChapter.getAttribute("phone_score"));
                resultJson.addProperty("ToneScore", readChapter.getAttribute("tone_score"));
                resultJson.addProperty("EmotionScore", readChapter.getAttribute("emotion_score"));
                resultJson.addProperty("TotalScore", readChapter.getAttribute("total_score"));
        ...
        return "{\"error\": \"Error parsing XML result\"}";
    }

    public void scoreSpeech(MultipartFile audioFile, String textToScore) throws Exception {
        System.out.println("即将评测文本是：" + textToScore);
        text = "[content]" + textToScore;
        String authUrl = getAuthUrl(hostUrl, apiKey, apiSecret);// 构建鉴权url
        OkHttpClient client = new OkHttpClient.Builder().build();
        System.out.println(authUrl);
        String url = authUrl.replace("http://", "ws://").replace("https://", "wss://");//将url中的 schema 分别替换为ws:// 和 wss://
        Request request = new Request.Builder().url(url).build();
        System.out.println("url===>" + url);
